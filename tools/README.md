...
- `set_version.py` — Update the repo `VERSION` file and optionally commit/tag (adds `-<branch>` suffix off `main`; tags by default on `main`). Flags: `--commit`, `--tag`, `--no-tag`, `--message`.
